...
const create = () => {
  ...
  const [metaData,setMetaData]=useState({name:"",description:"",target:"",amtNft:"",deadline:''});
...
/*   useEffect(()=>{
  ...
  const sendData=async()=>{
    ...
    try{
        ...
        try{
        const createCampaign=await contract.createCampaign(`https://ipfs.io/ipfs/${metaHash}`,metaData.name,toWei( metaData.target),toWei( metaData.amtNft),metaData.deadline);
  ...
  return (
    ...
    {!wallet?<Wallet/>:!chain?<Chain/>:<>
      ...
      <div className='w-3/5 text-lg text-slate-600' style={{fontFamily: "Quicksand, sans-serif"}}>
         ...
         <div className=''>
              <b className='text-xl'>Set End Date</b>
              <input type="date"  className='my-2 rounded-md w-full px-5 py-2 border-solid border-2 outline-none'  onChange={(event)=>{

                  if(checkDate(event.target.value)){const dateF=new Date(event.target.value).getTime();setMetaData({...metaData,["deadline"]:dateF})}
                                   
                  else{   window.alert("Give future Date"); }
